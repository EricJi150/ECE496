...
def import_data():
    ...
    for data_path in glob.glob(deepfloyd_outdoor_path + '/*'):
        classes.append(data_path.split('/')[-1])
        if mode == 'streets':
            deepfloyd_outdoor_image_paths.append(glob.glob(data_path + '/*'))
    ...
    test_image_paths = deepfloyd_outdoor_image_paths
